...
export const add = async (req, res) => {
	try {
		...
		const getAttributeQuery = "SELECT * FROM attributes WHERE id = ?";
		const attribute = await db.query(getAttributeQuery, attribute_id);

		if (!attribute) {
			throw new NotFound("Attribute not found");
		}

		const newValue = { value, attribute_id };
...
export const getOne = async (req, res) => {
	try {
		...
		const getQuery = `
      SELECT av.attribute_id, av.id AS attribute_value_id, a.name, av.value 
