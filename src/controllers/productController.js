...
export const getAll = async (req, res) => {
    try {
        ...
        const getTotalItemsQuery = `
            SELECT * FROM products_categories AS pc 
        ...
        console.log(getTotalItemsQuery);

        const [[{ "COUNT(p.id)": totalItems }]] = await db.query(getTotalItemsQuery, categoryId);

        const pagination = new Pagination(totalItems, page, limit);

        const getQuery = `
            SELECT p.id, p.name_uz AS product_name_uz, p.name_ru AS product_name_ru, p.desc_ru, p.desc_uz, 
