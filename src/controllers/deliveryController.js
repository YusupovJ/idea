...
export const add = async (req, res) => {
    try {
        ...
        const { address_id, deliver_id, delivery_fee, note } = req.body;
        ...
        const [{ insertId }] = await db.query(addQuery, newDelivery);

        const getTotalPriceQuery = `
            SELECT SUM(o.count * p.price) FROM orders AS o
            JOIN products AS p
            ON p.id = o.product_id
            WHERE o.delivery_id = ?
        `;
        const [[{ "SUM(o.count * p.price)": total_price }]] = await db.query(getTotalPriceQuery, insertId);

        const updateQuery = "UPDATE delivery SET total_price = ? WHERE id = ?";
        await db.query(updateQuery, total_price, insertId);
...
export const getOne = async (req, res) => {
    try {
        ...
        const getQuery = `
            SELECT d.id AS delivery_id, d.deliver_id, d.total_price, d.note, d.created_at, d.updated_at,
            a.name AS address_name, a.city, a.region, a.street, a.house
            FROM delivery AS d 
            JOIN address AS a
            ON d.address_id = a.id
            WHERE id = ?
        `;
        const [[delivery]] = await db.query(getQuery, id);

        if (!delivery) {
            throw new NotFound("Delivery not found");
        }

        apiResponse(res).send(delivery);
    } catch (error) {
        apiResponse(res).throw(error);
    }
};
