...
export const getAll = async (req, res) => {
	try {
		...
		const getTotalItemsQuery = `SELECT COUNT(id) FROM address  WHERE user_id = ? AND name LIKE '%${search}%'`;
		const [[{ "COUNT(id)": totalItems }]] = await db.query(getTotalItemsQuery, +userId);

		const { page, limit } = req.query;
		const pagination = new Pagination(totalItems, page, limit);

		const getQuery = `SELECT * FROM address WHERE user_id = ? AND name LIKE '%${search}%' LIMIT ? OFFSET ?`;
